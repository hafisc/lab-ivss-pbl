...
require_once __DIR__ . '/../../../app/models/MemberPublication.php';
require_once __DIR__ . '/../../../app/config/Database.php';

$db = Database::getInstance()->getPgConnection();
$pubModel = new MemberPublication($db);
$publication = $pubModel->getById($pubId);

// Verify ownership
$userId = $_SESSION['user_id'] ?? null;
if (!$publication || $publication['user_id'] != $userId) {
    $_SESSION['error'] = 'Publikasi tidak ditemukan atau Anda tidak memiliki akses.';
    header('Location: index.php?page=member-publications');
    exit;
}
?>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-slate-500 md:ml-2">Edit Publikasi</span>
                </div>
            </li>
        </ol>
    </nav>
...
<!-- Header -->
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-800 mb-1">Edit Publikasi</h1>
    <p class="text-sm text-slate-500">Perbarui informasi publikasi penelitian Anda</p>
</div>
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
    <form action="index.php?page=member-publication-update" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <input 
                    ...
                    id="authors" 
                    required 
                    value="<?= htmlspecialchars($publication['authors']) ?>"
                    class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input 
                        ...
                        id="journal" 
                        required 
                        value="<?= htmlspecialchars($publication['journal']) ?>"
                        class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                ...
                <div>
                    ...
                    <input 
                        ...
                        max="2100" 
                        required 
                        value="<?= htmlspecialchars($publication['year']) ?>"
                        class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input 
                        ...
                        name="volume" 
                        id="volume" 
                        value="<?= htmlspecialchars($publication['volume'] ?? '') ?>"
                        class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                        placeholder="Vol. 10"
                    >
                </div>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="issue" 
                        id="issue" 
                        value="<?= htmlspecialchars($publication['issue'] ?? '') ?>"
                        class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                        placeholder="No. 2"
                    >
                </div>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="pages" 
                        id="pages" 
                        value="<?= htmlspecialchars($publication['pages'] ?? '') ?>"
                        class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                        placeholder="100-115"
                    >
                </div>
            ...
            <div>
                ...
                <input 
                    ...
                    name="url" 
                    id="url" 
                    value="<?= htmlspecialchars($publication['url'] ?? '') ?>"
                    class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-3 flex-1">
                        <h3 class="text-sm font-medium text-blue-800">File Saat Ini</h3>
                        <div class="mt-2 text-sm text-blue-700">
                            <p class="flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                                </svg>
                                <span><?= basename($publication['file_path']) ?></span>
                                <a href="<?= htmlspecialchars($publication['file_path']) ?>" target="_blank" class="text-blue-600 hover:text-blue-800 underline">Lihat</a>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div>
                ...
                <p class="mt-2 text-xs text-slate-500">Biarkan kosong jika tidak ingin mengubah file PDF</p>
            </div>
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-amber-700">
                            <ul class="list-disc list-inside space-y-1">
                                ...
                                <li>File PDF baru akan menggantikan file lama jika diupload</li>
                                <li>Perubahan tidak dapat dibatalkan setelah disimpan</li>
                            </ul>
                        </div>
                    </div>
                </div>
